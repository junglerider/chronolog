export default new Map([
  // language menu ----------------------------------------
  ['English', 'Inglese'],
  ['German', 'Tedesco'],
  ['French', 'Francese'],
  ['Spanish', 'Spagnolo'],
  ['Italian', 'Italiano'],
  // sidebar ----------------------------------------------
  ['Dashboard', 'Pagina iniziale'],
  ['Time clock, status and summary', 'Orologio da controllo, status e sommario'],
  ['Change Password', 'Cambia la password'],
  ['Change your password', 'Cambia la password'],
  ['Logout', 'Disconnettersi'],
  ['Forget password and display login form', 'Dimentica la password e visualizza il modulo di accesso'],
  ['Time Sheets', 'Scheda attività'],
  ['Edit Time Sheet', 'Modifica scheda attività'],
  ['Enter working time and hours on a daily basis', 'Inserisci l\'orario di lavoro e le ore su base giornaliera'],
  ['Weekly Overview', 'Compendio settimanale'],
  ['See working time entries for a week', 'Visualizza l\'orario di lavoro della settimana'],
  ['Monthly Overview', 'Compendio mensile'],
  ['See working time entries for a whole month', 'Visualizza l\'orario di lavoro per l\'intero mese'],
  ['Todo List', 'Lista di cose da fare'],
  ['See/edit todo list, add tasks', 'Vedi/modifica todo list, aggiungi compiti'],
  ['Contacts', 'Contatti'],
  ['Contact', 'Contatto'],
  ['Look up people and edit contact data', 'Cerca le persone e modifica i dati di contatto'],
  ['Organisations', 'Organizzazioni'],
  ['Organisation', 'Organizzazione'],
  ['Look up organisations and edit contact data', 'Cerca le organizzazioni e modifica i dati delle organizzazioni'],
  ['Reporting', 'Dati e resconti'],
  ['Reports', 'Resconti'],
  ['Report', 'Resconto'],
  ['Select report to view/print from a list of reports', 'Seleziona risconto per visualizzare/stampare da un elenco di risconti'],
  ['Invoices', 'Fatture'],
  ['View, edit, create and print invoices', 'Visualizza, modifica, crea e stampa fatture'],
  ['Admin', 'Amministrazione'],
  ['Administrator', 'Amministratore'],
  ['Projects & Tasks', 'Progetti e compiti'],
  ['View project tree and maintain tasks', 'Visualizza la gerarchia del progetto e modifica i compiti'],
  ['Employees', 'Dipendenti'],
  ['Customers', 'Clienti'],
  ['View/edit customers', 'Visualizza/modifica i clienti'],
  ['Users', 'Utenti'],
  ['User', 'Utento'],
  ['View/edit users and access privileges', 'Visualizza/modifica gli utenti ed i privilegi di accessso'],
  ['Correct Time Logs', 'Correggi registro orario'],
  ['Amend time log entries of employees', 'Modifica le voci del registro orario dei dipendenti'],
  // data tables  -----------------------------------------
  ['Actions', 'Azioni'],
  ['Search', 'Ricerca'],
  ['No data to display', 'Nessuna datta da mostrare'],
  ['Add', 'Inserisci'],
  ['Delete', 'Cancellare'],
  ['Are you sure you want to delete this record?', 'Vuoi cancellare questo record?'],
  ['Are you sure you want to delete {1} records?', 'Vuoi cancellare questi {1} record?'],
  ['Are you sure you want to delete this entry?', 'Cancellare questa voce?'],
  ['OK - Deleted!', 'OK - cancellato!'],
  ['Deletion did not succeed.', 'I dati non possono essere cancellati.'],
  ['OK - Saved!', 'OK - salvato!'],
  ['Saving did not succeed.', 'Non è stato possibile salvare i dati.'],
  ['No changes were made.', 'I dati non sono stati modificati.'],
  //  components  and buttons -----------------------------
  ['View/Edit', 'Visualizza/modifica'],
  ['Edit', 'Modifica'],
  ['Save', 'Salva'],
  ['Cancel', 'Annulla'],
  ['Update', 'Aggiorna'],
  ['Reset', 'Ripristina'],
  ['Print', 'Stampa'],
  ['Duplicate', 'Duplica'],
  ['Password', 'Password'],
  ['Login', 'Accedere'],
  ['Next month', 'Prossimo mese'],
  ['Today', 'Oggi'],
  ['Date must be in the past or present', 'La data deve essere nel passato o nel presente'],
  ['Date must be in the future', 'La data deve essere in futuro'],
  ['Invalid date', 'Data non valida'],
  ['No results', 'Nessun risultato'],
  ['Required', 'Input richiesto'],
  ['yes', 'sì'],
  ['no', 'no'],
  //  dashboard -------------------------------------------
  ['Welcome back', 'Ben tornato'],
  ['Status', 'Stato'],
  ['present', 'presente'],
  ['absent', 'assente'],
  ['Arrival today', 'Venuta oggi'],
  ['Working time today', 'Ore registrate oggi'],
  ['Departure', 'Partenza'],
  ['Hours today', 'Ore totali oggi'],
  ['Hours this week', 'Ore totali questa settimana'],
  ['Hours this month', 'Ore totali questo mese'],
  ['Check in', 'Check-in'],
  ['Check out', 'Check-out'],
  // contacts + organisations -----------------------------
  ['Contact Details', 'Dettagli del contatto'],
  ['Add Person', 'Inserisci contatto'],
  ['Add Organisation', 'Inserisci organizzazione'],
  ['Organisation Details', 'Dettagli dell\'organizzazione'],
  ['Type', 'Tipo'],
  ['Entry', 'Voce'],
  ['Title', 'Saluto'],
  ['First Name', 'Nome'],
  ['Last Name', 'Cognome'],
  ['Nick Name', 'Soprannome/Alias'],
  ['First, last or nick name must be given', 'Nome, cognome o s soprannome deve essere inserito'],
  ['Street Address', 'Strada/numero civico'],
  ['Postal Code', 'Codice postale'],
  ['City', 'Città'],
  ['State/Province', 'Stato/provincia'],
  ['Country', 'Paese'],
  ['First Contact', 'Primo contatto'],
  ['Last Contact', 'Ultimo contatto'],
  ['Updated at', 'Ultimo salvataggio su'],
  ['Name', 'Nome'],
  ['Industry', 'Settore industriale'],
  ['Position', 'Posizione'],
  ['Notes', 'Appunti'],
  ['Customer', 'Cliente'],
  ['Supplier', 'Fornitore'],
  ['Private', 'Privato'],
  ['Prospect', 'Potenziale cliente'],
  ['Employee', 'Dipendente'],
  ['Associate', 'Socio'],
  ['Government', 'Autorità'],
  ['Non-Government', 'Non-governo'],
  ['Phone/Email', 'Telefono/e-mail'],
  ['Phone', 'Telefono'],
  ['Mobile', 'Cellulare'],
  ['Email', 'E-mail'],
  ['Fax', 'Fax'],
  ['Office', 'Ufficio'],
  ['Home', 'Casa'],
  ['Website', 'Sito web'],
  ['Members/Employees', 'Membri/dependenti'],
  ['Invalid e-mail address', 'E-mail non valido'],
  ['Invalid phone number', 'Numero di telefono non valido'],
  ['Open', 'Apri'],
  // time sheets + todo list ------------------------------
  ['Mon', 'lun'],
  ['Tue', 'mar'],
  ['Wed', 'mer'],
  ['Thu', 'gio'],
  ['Fri', 'ven'],
  ['Sat', 'sab'],
  ['Sun', 'dom'],
  ['Monday', 'lunedì'],
  ['Tuesday', 'martedì'],
  ['Wednesday', 'mercoledì'],
  ['Thursday', 'giovedì'],
  ['Friday', 'venerdì'],
  ['Saturday', 'sabato'],
  ['Sunday', 'domenica'],
  ['January', 'Gennaio'],
  ['February', 'Febbraio'],
  ['March', 'Marzo'],
  ['April', 'Aprile'],
  ['May', 'Maggio'],
  ['June', 'Giugno'],
  ['July', 'Luglio'],
  ['August', 'Agosto'],
  ['September', 'Settembre'],
  ['October', 'Ottobre'],
  ['November', 'Novembre'],
  ['December', 'Dicembre'],
  ['Total hours', 'Ore totali'],
  ['Daily Time Sheet', 'Scheda attività quotidiana'],
  ['Task', 'Compito'],
  ['Description', 'Descrizione'],
  ['Hours', 'Ore'],
  ['Must be a number', 'Deve essere un numero'],
  ['Must be larger than zero', 'Deve essere maggiore di zero'],
  ['Created', 'Creato'],
  ['Created at', 'Creato'],
  ['Task Details', 'Dettagli del compito'],
  ['Active', 'Attivato'],
  ['Project', 'Progetto'],
  ['Some tasks have hours booked. Tasks with hours booked cannot be deleted.', 'Alcuni compiti hanno assegnato orari di lavoro. Questi compiti non possono essere eliminati.'],
  ['The customer has been changed.', 'Il cliente assegnato è stato modificato.'],
  ['Period', 'Periodo'],
  ['Start date', 'Data d\'inizio'],
  ['End date', 'Data di fine'],
  ['Assigned to me', 'Assegnato a me'],
  ['logged', 'ore/giorno'],
  ['clocked', 'cronometrato'],
  ['Invalid time', 'Inserimento non valido'],
  ['Must not be zero', 'Non deve essere 0'],
  ['May not exceed {1} hrs', 'Non può superare le {1} ore'],
  ['Reassign', 'Riassegnare'],
  ['Reassign this task to', 'Riassegna quest compito a'],
  ['Time sheet entries for this task', 'Voci della scheda attività per questo compito'],
  ['Move', 'Spostare'],
  ['Move selected entries to a different date', 'Sposta le voci selezionate in una data diversa'],
  // customers and users ----------------------------------
  ['Is Retired', 'Inattivo'],
  ['Customer Details', 'Dettagli cliente'],
  ['Number of tasks', 'Numero di compiti'],
  ['Some customers have tasks assigned. Customers with tasks cannot be deleted.', 'Sono stati inseriti compiti per alcuni clienti. Questi clienti non possono essere eliminati.'],
  ['Is Active', 'È attivo'],
  ['User name', 'Nome utente'],
  ['Some users have tasks or time sheets assigned. Users with tasks or time sheets cannot be deleted.', 'Alcuni utenti hanno compiti o attività. Questi utenti non possono essere eliminati.'],
  ['User Details', 'Dettagli utente'],
  ['Last visit', 'Ultima sessione'],
  ['Total visits', 'Numero totale di sessioni'],
  ['Number of time sheet entries', 'Numero di scheda attività'],
  ['This user name already exists.', 'Questo nome utente esiste già.'],
  ['Access control', 'Diritti di accesso'],
  ['Reset password', 'Resetta la password'],
  ['This account is not password-protected.', 'Questo account utente non è protetto da password.'],
  [`Delete the user's password and allow access without password?`, 'Eliminare la password dell\'utente e consentire l\'accesso senza password?'],
  // login/password ---- ----------------------------------
  ['Old password', 'Password precedente'],
  ['New password', 'Nuova password'],
  ['Retype new password', 'Ripetere la nuova password'],
  ['Password is identical to previous one', 'La password precedente non può essere riutilizzata'],
  ['Insufficient password strength', 'La password non è sufficientemente sicura'],
  ['The retyped password is not the same', 'La password ridigitata non è la stessa'],
  ['The server did not accept the credentials.', 'Il server non ha accettato le credenziali.'],
  ['The server could not update the password.', 'Il server non ha potuto aggiornare la password.'],
  ['Your password has been updated.', 'La tua password è stata aggiornata.'],
  // reporting --------------------------------------------
  ['Chronological time sheet', 'Scheda attività cronologica'],
  ['Time sheet by task', 'Scheda attività in base ai compiti'],
  ['Project summary', 'Sintesi del progetto'],
  ['Project time sheet', 'Scheda attività del progetto'],
  ['Project time sheet by task', 'Scheda attività del progetto in base ai compiti'],
  ['Project time sheet by contributor', 'Scheda attività del progetto in base ai collaboratori'],
  ['Project/Task', 'Progetto/Compito'],
  ['Generate', 'Creare'],
  ['Calculate', 'Calcolare'],
  ['all', 'tutti'],
  ['All customers', 'Tutti i clienti'],
  ['All users', 'Tutti i utenti'],
  ['This week', 'Questa settimana'],
  ['Last week', 'L\'ultima settimana'],
  ['This month', 'Questo mese'],
  ['Last month', 'Lo scorso mese'],
  ['Last three months', 'Gli ultimi tre mesi'],
  ['Last six months', 'Gli ultimi sei mesi'],
  ['This year', 'Quest\'anno'],
  ['Last year', 'L\'anno scorso'],
  ['From', 'Da'],
  ['to', 'fino a'],
  ['Day', 'Giorno'],
  ['Arrival', 'Venuta'],
  ['Working time', 'Orario di lavoro'],
  // invoices ---------------------------------------------
  ['Invoice', 'Fattura'],
  ['Invoicing', 'Fatturazione'],
  ['Invoice Details', 'Dettagli di fatturazione'],
  ['Invoice items', 'Voci contabili'],
  ['Headline', 'Intestazione'],
  ['Invoice no.', 'Nº di fattura'],
  ['No.', 'Nº'],
  ['Cust. no.', 'Nº cliente'],
  ['Date', 'Data'],
  ['Due date', 'Data di scadenza'],
  ['Address', 'Indirizzo'],
  ['Payment terms', 'Termini di pagamento'],
  ['Issued by', 'Emittente'],
  ['Currency', 'Valuta'],
  ['Amount', 'Importo'],
  ['Quantity', 'Quantità'],
  ['Unit price', 'Prezzo unitario'],
  ['Total', 'Totale'],
  ['Net total', 'Importo netto'],
  ['Subtotal', 'Subtotale'],
  ['Grand total', 'Importo totale'],
  ['Sales tax', 'IVA'],
  ['Sales tax rate', 'Aliquota dell\'IVA'],
  ['Sales tax amount', 'Totale IVA'],
  ['All prices in', 'Tutti i prezzi sono in'],
  ['Invoice number already exists.', 'Questo numero di fattura esiste già.'],
  ['Authorised signature', 'Firma autorizzata'],
  ['draft', 'Creata'],
  ['sent', 'Pendente'],
  ['paid', 'Pagata'],
  ['overdue', 'Scaduta'],
  ['voided', 'Annullata'],
  ['days', 'giorni'],
  ['Invoice header', 'L\'intestazione della fattura'],
  ['Invoice footer', 'Piè de pagine della fattura'],
  ['Enter HTML for the printable invoice header', 'Contenuto HTML per l\'intestazione della fattura'],
  ['Enter HTML for the printable invoice footer', 'Contenuto HTML per el piè de pagine della fattura'],
  // projects and tasks -----------------------------------
  ['Projects', 'Progetti'],
  ['Active project', 'Progetto attivo'],
  ['Active task', 'Compito attivo'],
  ['Closed', 'Completato'],
  ['Closed project', 'Progetto completato'],
  ['Closed task', 'Compito completato'],
  ['Inactive project', 'Progetto inattivo'],
  ['Inactive task', 'Compito inattivo'],
  ['active', 'attivo'],
  ['inactive', 'inattivo'],
  ['Parent project', 'Progetto genitore'],
  ['Assigned to', 'Assegnato a'],
  ['Insert child task', 'Inserisci compito subordinato'],
  ['Close', 'Completare'],
  ['Reopen', 'Riaprire'],
  ['Activate', 'Attivare'],
  ['Deactivate', 'Disattivare'],
  ['Cannot move project to descendant.', 'Il progetto non può essere spostato in un ramo secondario.'],
  ['The customers are different. Move anyway?', 'I clienti sono diversi. Spostarlo comunque?'],
])