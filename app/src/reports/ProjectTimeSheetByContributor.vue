<template>
    <div class="report-container">
      <div class="report-title">{{ "Project time sheet by contributor" | i18n }}</div>
    <div>
  ...
  methods: {
    groupData() {
      ...
      for (let rec of this.data) {
        if (!groupedData.has(rec.user_name)) {
          groupedData.set(rec.user_name, {
            data: [],
            total: 0,
            contributor: rec.user_name,
          })
        }
        const section = groupedData.get(rec.user_name)
